...
- **Constellation Scramble is live:** two players race to make the same target on separate server-authoritative boards. Every attempted, successful, and rejected pairing is visible to both competitors, so reading and manually reusing a rival's route is part of the game.
- **Private and public play have different stakes:** single-use friend invitations are unranked. Public matchmaking unlocks after one scored solo win and changes a separate seasonal Duel Rating; it never changes solo Route Rank, Stardust, mastery, events, or rewards.
...
- **The opening has new wide and phone cuts:** landscape and portrait launches now use the new Kintsugi studio ident followed by the original rocket sequence, joined through black as one stereo film. The phone cut adds a pure-black pre-roll and gentle fade-in before the ident. Every play remains at 1× speed.
...
- **Seven independent cosmetic surfaces:** word plaques, movement/fusion trails, board finishes, main-menu backgrounds, opening gates, menu finishes, and sound themes with original looping scores, synchronized gameplay pulses, and themed cue banks can be equipped as full collections or mixed without changing gameplay.
- **Eight complete collections:** Celestial Atlas is the free foundation; Aurora Archive, Solar Foundry, Lunar Garden, and Eclipse Sovereign retain their coordinated Supporter identities; Pixel Frontier adds authentic 16-bit adventure styling, Bubble Reef is an original whimsical undersea-cartoon world, and Stellar Vanguard is an original cinematic space-opera sanctuary. Eclipse Sovereign remains the sole prestige full-shell transformation. Every kit includes responsive Home and opening Gate backgrounds, and Cartographer, First Light, and Weekly Sigil pieces are earned from verified play.
- **A real Cosmetics Observatory:** Collections, Pieces, and Owned views support locked previews, one-action kits, individual pieces, responsive Home/Gate art, collection-aware share cards, sound samples, apply/cancel, and Full/Reduced/Off effects.
...
- **Release-safe art delivery:** responsive packs load on demand into their own cache, returning players preload only the equipped Home/Gate set, and server, Pages, and itch builds enforce portable paths, exact dimensions, and pack budgets.
...
- **Play now means play:** the main Play action immediately closes the Cosmic Gate and prepares the next real challenge behind a readable quote. When the doors open, one small objective card gives the target, one plain instruction, any challenge-ending limit, and one Start action. Timers, moves, and scoring wait until Start.
- **Every challenge uses the same arrival:** normal word journeys and optional Star Break activities enter through the same gate instead of appearing from an unrelated menu or result-screen jump.
...
- **Every journey now opens with a thought:** launch and new-level Cosmic Gates pause long enough to present one of 50 concise lines about curiosity, creation, and discovery. Original Constellore writing is joined by a small, sourced public-domain collection, with immediate repeats prevented. Pause and Escape stay instant.
...
- **Play now travels through the Cosmic Gate:** after the mission summary, the doors close over the menu, the board is prepared while hidden, and the doors reopen onto the words. Timed challenges do not begin counting down until the board is visible. Launch and level entry now hold a readable quote before the doors open; Pause and Escape are deliberately unaffected.
- **The cosmos stays sharp at every size:** the Cosmic Gate now uses purpose-built portrait, 1080p, and 4K-sized responsive artwork instead of stretching one 1088px file across every screen. The browser selects the right source for the device, while the home panorama stays anchored to the viewport so a taller page cannot pull the art apart.
...
- **The Cosmic Gate gives every session an arrival:** launch begins on a layered blue-and-gold cosmos, a vertical seal draws through the sky, and two parallax doors open onto the game. Pause and completed-game results reuse the same visual language, with reduced-motion and forced-colors alternatives. Pause now keeps Continue, Restart, Show path, and Quit in one place, and labels Show path as a no-points choice before it runs.
...
- **Quick board controls** - Undo, Redo, Tidy, and Clear stay visible on the board; synchronized master-volume controls remain available in every level and Pause; Quit returns Home in one click while Restart keeps its confirmation.
- **Combination story** - successful recipes build a compact visual diorama behind the board words, with a persistent foundation, category-shaped chapters, accessible narration, and a non-destructive crumble/rebuild response for wrong pairs.
...
- **Personal Universe** - unique discoveries, Stardust, ranks, wins, streaks, and cosmetic loadout persist on this device. The beta does not connect gameplay progress to cloud saving.
...
**Make a Wish** introduces one recognizable concept into a run. It is intentionally an uncertain shortcut rather than an answer button. Every guest receives one free personal Wish. Wishes and other gameplay assistance are earned or granted through play; they are not included in a cash product. The disabled Supporter Pack prototype contains only complete presentation kits: word plaques, movement and fusion trails, board finishes, main-menu backgrounds, opening gates, menu finishes, and sound themes. Do not enable checkout until the store description, terms, fulfillment, and recovery behavior match that cosmetic-only offer.
