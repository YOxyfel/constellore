...
test("a missing combination asks for the player's expected result and sends the bounded report", async ({ page, browserName }) => {
  ...
  await expect(feedback.locator("#expectedPairButton")).toHaveText("Idea sent");
  expect(new Set([reportPayload.a, reportPayload.b])).toEqual(new Set(["Earth", "Water"]));
