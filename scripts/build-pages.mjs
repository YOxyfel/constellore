...
gameHtml = gameHtml
  ...
  .replaceAll('href="/styles.css?v=2.6.1"', 'href="./styles.css?v=2.6.1"')
  .replaceAll('src="/app.js?v=1.13.1"', 'src="./app.js?v=1.13.1"');
...
await copyFile(join(root, "public", "ctrl-hover.mjs"), join(playOutput, "ctrl-hover.mjs"));
await copyFile(join(root, "public", "shift-board.mjs"), join(playOutput, "shift-board.mjs"));
await copyFile(join(root, "public", "frictionless.mjs"), join(playOutput, "frictionless.mjs"));
await copyFile(join(root, "public", "mission-briefing.mjs"), join(playOutput, "mission-briefing.mjs"));
await copyFile(join(root, "public", "local-beta.mjs"), join(playOutput, "local-beta.mjs"));
await copyFile(join(root, "public", "cosmic-twists.mjs"), join(playOutput, "cosmic-twists.mjs"));
await copyFile(join(root, "public", "recipe-mastery.mjs"), join(playOutput, "recipe-mastery.mjs"));
await copyFile(join(root, "public", "engagement-features.mjs"), join(playOutput, "engagement-features.mjs"));
await copyFile(join(root, "public", "first-orbit.mjs"), join(playOutput, "first-orbit.mjs"));
await copyFile(join(root, "public", "universe-director.mjs"), join(playOutput, "universe-director.mjs"));
await copyFile(join(root, "public", "constellation-card.mjs"), join(playOutput, "constellation-card.mjs"));
await copyFile(join(root, "public", "cosmetic-economy.mjs"), join(playOutput, "cosmetic-economy.mjs"));
await copyFile(join(root, "public", "recipe-feedback.mjs"), join(playOutput, "recipe-feedback.mjs"));
await copyFile(join(root, "public", "pending-scores.mjs"), join(playOutput, "pending-scores.mjs"));
...
const serviceWorker = `const CACHE = "constellore-pages-practice-v22";
const BASE = new URL("./", self.location.href);
const SHELL = ["./", "./styles.css?v=2.6.1", "./app.js?v=1.13.1", "./ctrl-hover.mjs?v=1.0.0", "./shift-board.mjs?v=1.0.0", "./frictionless.mjs?v=1.0.0", "./mission-briefing.mjs?v=1.0.1", "./local-beta.mjs", "./local-world.mjs", "./cosmic-twists.mjs", "./recipe-mastery.mjs?v=1.0.0", "./engagement-features.mjs?v=1.1.0", "./first-orbit.mjs?v=1.0.0", "./universe-director.mjs?v=1.0.0", "./constellation-card.mjs?v=1.0.0", "./cosmetic-economy.mjs?v=1.0.0", "./recipe-feedback.mjs?v=1.0.0", "./pending-scores.mjs?v=1.0.0", "./manifest.webmanifest", "./icon.svg"].map((path) => new URL(path, BASE).href);
