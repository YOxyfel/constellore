...
| Collection | Access | Design language | Home presentation | Full-collection status |
| --- | --- | --- | --- | --- |
...
Every complete collection includes responsive Home scenes and Constellation Fold signatures. All collections keep the established information architecture and change only their owned presentation surfaces. Eclipse Sovereign remains the sole full-shell treatment: its coordinated `homeScene` and `uiFinish` transform the surrounding shell, borders, effects, and interface finish while leaving the same controls and content understandable.
...
The player-facing Cosmetic Lab (implemented by the stable Observatory module) supports:

- An always-visible equipped-look rail with one shortcut per cosmetic slot.
- Separate Collections and Pieces views with an independent Owned-only inventory filter.
- Locked previews without accidental equip.
- One-action full-collection equip and independent piece equip.
- Custom-look summaries and collection-aware share cards.
- Real Home/Worldweave preview artwork, piece-specific thumbnails, and sound samples.
- Independent music, word/fusion, and haptic switches plus master, music, and SFX volume controls.
- Apply/cancel semantics with the equipped look restored on cancel.
...
- Each slot owns only its surface. A Solar UI finish cannot replace an Aurora Home scene or Constellation Fold.
...
Home scenes and Worldweave materials ship in responsive `sm`, `md`, and `lg` WebP variants. Generated masters and prompt provenance are recorded in `ART_ASSET_PROVENANCE.md`. Original soundtrack loops, synchronized gameplay-pulse stems, and themed SFX banks are recorded in `AUDIO_ASSET_PROVENANCE.md` and remain outside the install shell.
...
- Use an early saved-look preload selector so returning players fetch one Home set and one Worldweave set instead of downloading default and equipped art.
...
3. Collection and custom-look CSS remain slot-independent.
4. Locked, unknown, and wrong-slot IDs fail closed.
5. Desktop and mobile Cosmetic Lab flows pass keyboard, focus, and visual review.
...
3. Produce Home/Worldweave source masters and responsive derivatives.
